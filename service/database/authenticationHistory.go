...
func (a *authenticationHistory) Create(model *AuthenticationHistoryModel) error {
	_, err := a.db.Exec("INSERT INTO authentification_history (user_id, logged_in, meta) VALUES(?,?,?)",
...
func (a *authenticationHistory) Update(model *AuthenticationHistoryModel) error {
	...
	_, err := a.db.Exec("UPDATE authentification_history SET user_id = ?, logged_in = ?, meta = ?, logged_out = ?  WHERE id = ? ",
