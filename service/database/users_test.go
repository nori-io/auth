...
func TestUsers_Create_userEmail(t *testing.T) {
	...
	mock.ExpectPrepare("INSERT INTO").
		ExpectExec().
		WithArgs("active", "vendor", AnyTime{}, AnyTime{}).
...
/*	rows := sqlmock.NewRows([]string{"id"}).
	...
	/*mock.ExpectPrepare("INSERT INTO auth").
		ExpectExec().
	...
	if 1==1 {
		...
		stmt, err1 := (mockDatabase.Prepare("INSERT INTO auth (user_id,  email, password, salt, created, updated, is_email_verified, is_phone_verified) VALUES(?,?,?,?,?,?,?,?)"))
